...
화면과 기능의 정본은 2026-09-12 피그마 화면 15장이다. 화면 구조와 문구는 피그마를 따르고 화면 구조를 넘어서는 동작 규칙은 2026-09-10 IA 기능 명세서를 참고한다.
...
| 요소                                     | 화면                 | 기다리는 결정                                                       |
| ---------------------------------------- | -------------------- | ------------------------------------------------------------------- |
| 연계 카페와 제휴 할인                    | 코스 결과 타임라인   | 팝업이 아닌 장소를 코스에 넣는지                                    |
| 방문 전 1시간 푸시                       | 저장 완료            | 푸시를 넣는지. 헤더의 알림 벨은 자리표시였다고 확인돼 그리지 않는다 |
| 방문 후기 탭과 평점, 리뷰 수, 조회수     | 상세, 비회원 홈 카드 | 후기와 평점, 조회수 데이터의 출처                                   |
| 대기시간 예상                            | 코스 결과 타임라인   | 대기시간 예상의 출처                                                |
| 마커 미리보기 카드의 현재 위치 도보 시간 | 탐색 지도            | 도보 시간 기준점                                                    |
...
- 플래너 조건 입력에 방문 날짜와 시작 시각을 넣는다. 피그마에는 없고 제출문과 IA에는 있다. 팝업에 운영 기간이 있어 날짜 없이는 어느 날 열린 곳을 고를지 정해지지 않는다
- 상세의 예약 링크는 `reservationUrl`이 있을 때 정보 영역에 외부 링크로 보인다. 피그마 하단 액션은 찜과 "이 팝업으로 AI 코스 추천받기" 둘이고 제출문은 예약 버튼을 적었다

## 화면과 라우트

라우트 그룹 둘로 하단 탭바 노출을 구조로 정한다. `(tabs)` 그룹의 레이아웃이 탭바를 그리고 `(flow)` 그룹에는 탭바가 없다. 탭바를 어느 화면에 노출할지는 미결정이고 결정이 바뀌면 폴더를 옮기는 것으로 끝난다. 반응형 범위는 `docs/design/DESIGN.md`에 있다.
...
구글 콜백 말고는 다 있다. 랜딩과 로그인, 홈, 탐색, 내 팝업은 내용이 있고 나머지 일곱은 `ScreenPlaceholder`로 자리만 있다.

| 경로                          | 화면             | 로그인    | 데이터를 받는 곳                                                | 기능 문서                  |
| ----------------------------- | ---------------- | --------- | --------------------------------------------------------------- | -------------------------- |
| `/`                           | 랜딩             | 아니오    | 없음. 정적                                                      | `onboarding.md`            |
| `/onboarding/[step]`          | 온보딩 1, 2, 3   | 예        | 서버                                                            | `onboarding.md`            |
| `/onboarding/result`          | 추천 미리보기    | 미결정    | 존치 여부가 ROADMAP 미결정에 있다                               | 없음                       |
...
로그인 필요 화면과 동작은 클라이언트에서 막는다. 토큰이 메모리에만 있는 동안은 서버가 읽을 세션이 없어 `proxy.ts`가 판단할 근거가 없기 때문이다. 비로그인 사용자가 찜이나 코스 만들기를 누르면 그 시점에 `/login?next={돌아올 경로}`로 보낸다. 로그인 뒤 `next`로 돌아온다.
...
| 위치                             | 담는 것                                                                                                                  |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `shared/model/region.ts`         | `Region` 다섯과 라벨 대응표. 온보딩과 탐색, 플래너, 팝업 필드가 같은 타입을 쓴다                                         |
| `shared/model/popup.ts`          | 카테고리와 예약 유형, 라벨 대응표, 카드가 그리는 값 `PopupCardItem`. 카드가 있는 모든 기능이 쓴다                        |
...
| `shared/lib/kakao-map`           | 마커와 마커 전체가 보이게 맞추기(`fitTo`)는 있다. 폴리라인과 번호 마커, 경로 좌표를 SDK 좌표로 옮기는 함수가 더 필요하다 |
| `shared/ui`                      | 화면 뼈대가 쓰는 열 개가 있다. 이 설계가 더 요구하는 것은 BottomSheet와 Tabs, Skeleton, ErrorState                       |
...
| `POST /api/v1/recommendations/preview` | 없음 | 추천 미리보기. 답을 받아 스코어와 예시를 낸다. 화면 존치가 미결정이다 | 없음                |
...
| `areaCode` 값                                | `SEONGSU`, `YEOUIDO`, `HONGDAE`, `SINCHON`, `YONGSAN` 다섯으로 고정                                                                                                             | `shared/model/region` |
| 리프레시 토큰 `Set-Cookie`                   | 로그인과 재발급 응답에서 리프레시 토큰을 httpOnly 쿠키로. `/refresh`와 `/logout`이 쿠키를 읽는다                                                                                | `auth.md`             |
...
| 비로그인 접근 처리          | 찜과 코스 만들기를 누른 시점에 `/login?next=`                                                                                                          | `auth.md`의 가드                                                                        |
| 지역 목록                   | 다섯. 피그마의 한남동과 잠실은 뺐다                                                                                                                    | `shared/model/region.ts`의 값 추가                                                      |
| 플래너 날짜                 | 날짜와 시작 시각을 받는다                                                                                                                              | `planner.md`의 폼 필드와 요청 타입                                                      |
| 연계 카페                   | 코스 항목의 `kind`가 `POPUP` 하나. `PLACE`가 오면 그린다                                                                                               | `course.md`의 `CourseItem`                                                              |
| 방문 전 푸시                | 저장 완료의 알림받기 자리만. 동작 없음                                                                                                                 | `course.md`의 저장 완료                                                                 |
| 후기와 평점, 조회수         | `reviewSummary`와 `viewCount`가 `null`이면 그리지 않는다                                                                                               | `popup.md`의 `PopupDetail`                                                              |
| 대기시간 예상               | `waitEstimateMinutes`가 `null`이면 그리지 않는다                                                                                                       | `course.md`의 `CourseItem`                                                              |
| 마커 도보 시간 기준점       | `walkFromLandmark`가 `null`이면 그리지 않는다. 위치 권한은 받지 않는다                                                                                 | `popup.md`의 마커 카드                                                                  |
| 탭바 노출 화면              | `(tabs)` 넷에만                                                                                                                                        | 라우트 그룹 폴더 이동                                                                   |
| 취향 스코어 축              | 서버가 `axis` 문자열로 내려주고 FE는 받은 대로 그린다                                                                                                  | 미리보기 화면의 `PreviewResult`                                                         |
| 기준 화면                   | 피그마 15장                                                                                                                                            | 이 문서 전체                                                                            |
| 폴리라인 좌표를 응답에 싣나 | 구간 응답에 실린다(FE 제안)                                                                                                                            | `course.md`의 `WalkSegment`                                                             |
| 카카오 REST 키 소유         | 토큰 교환은 백엔드 몫. 다만 프론트의 authorize 호출이 같은 앱의 REST API 키를 `client_id`로 써야 교환이 통과한다. 지금 두 앱이 달라 로그인이 막혀 있다 | `NEXT_PUBLIC_KAKAO_CLIENT_ID`를 백엔드 앱의 키로 바꾸고 그 앱에 Redirect URI를 등록한다 |
| 토큰 보관                   | 메모리. 쿠키가 오면 리프레시 토큰 필드를 스토어에서 뺀다                                                                                               | `auth.md`                                                                               |
| 캘린더 방식                 | 구글 URL 템플릿과 .ics(FE 제안)                                                                                                                        | `course.md`의 캘린더 절                                                                 |

이 설계로 닫힌 것도 있다. 추천 실패 화면은 인기순 대체와 라벨, 마커 상한은 백엔드 페이지 상한 50, 온보딩 건너뛰기는 신호 하나 필수로 정했다. 온보딩 답을 로컬에 보관하는 처리는 로그인이 온보딩보다 앞으로 오면서 필요가 없어졌다.

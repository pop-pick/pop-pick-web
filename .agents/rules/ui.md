---
description: 기성 UI 라이브러리 없음. 공용 컴포넌트는 src/shared/ui가 주인. 모바일 퍼스트, 키보드로 조작 가능, 토큰만 쓴다
---
...
디자인 토큰을 아직 받지 못했다. 다크 모드 지원 여부와 기준 폭, 컴포넌트 파일 이름 규칙도 정해지지 않았다. 정해지면 이 룰을 고친다. 자세한 안내는 `src/shared/ui/README.md`에 있다.

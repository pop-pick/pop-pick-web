---
description: 서버 호출은 src/shared/api를 거친다. API 규약 합의 전에는 fetch 래퍼 하나로, 실패는 쿼리 에러 상태로 드러낸다
---
...
응답 공통 구조와 에러 코드 체계, 날짜와 시간 포맷, 페이지네이션, 필드 이름 규칙, 스펙 문서 위치 여섯 항목은 8/30 개발자 논의에서 정한다. 정해지면 이 절을 정해진 내용으로 바꾼다. 그 전에는 그럴듯한 기본값을 채우지 않는다. 항목별 설명은 `src/shared/api/README.md`에 있다.

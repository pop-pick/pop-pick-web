---
description: main과 develop, feature 3단 브랜치와 squash 머지 금지. 커밋 메시지 형식과 타입, lefthook 훅 셋, PR 머지 조건과 릴리스 뒤 정리
---
...
타입은 소문자 영문이고 제목은 50자 이내의 한국어다. 한 태스크가 한 커밋이라 성격이 다른 변경을 한 커밋에 섞지 않는다.
...
여기에 함정이 하나 더 있다. **한 파일에 스테이징된 변경과 스테이징되지 않은 변경이 함께 있으면 훅 처리 중 스테이징되지 않은 변경이 사라질 수 있다.** `git status --short`로 `MM`으로 시작하는 줄을 먼저 확인하고 걸리는 파일이 있으면 정리한 뒤에 커밋한다. 커밋한 뒤에도 `git status`를 다시 대조해 의도한 것만 들어갔는지 본다.
...
1. **`main`과 `develop` 직접 커밋.** 항상 feature 브랜치를 경유한다
2. **`git add -A` 광범위 스테이징.** 파일 단위로 명시해서 add한다
3. **`.env`와 비밀값 파일 커밋.** `.gitignore` 확인이 필수다
4. **병합 충돌 `--ours` 일방 해소.** 양쪽 의미를 검토한 뒤 해소한다
5. **강제 푸시.** 리베이스가 필요하면 `--force-with-lease`만 쓰고 기본 브랜치에는 쓰지 않는다

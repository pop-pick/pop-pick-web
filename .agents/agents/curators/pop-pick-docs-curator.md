...
무엇을 어느 문서에 두는지는 `docs/README.md`의 배치 기준이 정한다. 한 사실은 한 문서에만 적고 나머지는 그 문서를 이름으로 가리킨다. 미결정은 `docs/product/ROADMAP.md`의 미결정 절 하나에만 둔다.
...
- 개인 경로와 팀 위키 위치
- 결정 이력의 부산물. 언제 누가 정했는지는 위키에 남는다

## 하지 않는 것

- 팀 위키를 고친다. 위키는 읽기 전용이다. 위키에 남길 것이 생기면 보고에 적어 사용자에게 넘긴다
- 시간순 기록을 소급해 고친다. ROADMAP의 하지 않기로 한 것 표가 그렇다
- 코드 수정과 커밋, 푸시
